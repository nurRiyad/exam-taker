...
- **Auth**: JWT returned as a bearer token (sent via `Authorization: Bearer` header, stored client-side in a non-httpOnly cookie), PBKDF2-SHA256 password hashing via Web Crypto — not bcrypt/argon2 (Workers CPU limits). Signup requires username, BD phone, password confirmation, and student/teacher role; name/email are profile-later fields. Login/reset identifiers are username or phone only. JWT claims are a UI convenience only; authorization checks always hit the DB (ADR-0054, ADR-0064, ADR-0065).
...
- **Workspace commands** (run from repo root): `pnpm install`, `pnpm dev` (both apps, parallel), `pnpm dev:api` / `pnpm dev:web` (one app), `pnpm db:generate` / `pnpm db:migrate:local` / `pnpm db:studio` (see `.claude/skills/d1-schema`; full first-time-setup vs. resuming walkthrough in `docs/technical-design.md`'s Local Development section).
